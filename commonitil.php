...
abstract class CommonITILObject extends CommonDBTM
{
   ...
   // STATUS
    ...
    public function showForm($ID, array $options = [])
    {
        ...
        // Kyndryl Customization Start - 2129745 - Template field for Problem/Ticket
        // Kyndryl Customization Start - Template Selection.
        if($this->getType() == 'Change') {
            $tt = $this->getITILTemplateToUse(
                $this->input['change_template'] ?? $options['template_preview'] ?? 0, 
                $type, 
                ($ID ? $this->fields['itilcategories_id'] : $options['itilcategories_id']), 
                ($ID ? $this->fields['entities_id'] : $options['entities_id']) 
            );
        } else if ($this->getType() == 'Problem') {
            $tt = $this->getITILTemplateToUse(
                $this->input['problem_template'] ?? $options['template_preview'] ?? 0, 
                $type, 
                ($ID ? $this->fields['itilcategories_id'] : $options['itilcategories_id']), 
                ($ID ? $this->fields['entities_id'] : $options['entities_id']) 
            );
        }
        // Kyndryl Customization End - Template Selection.
        // Kyndryl Customization End - 2129745 - Template field for Problem/Ticket

        $predefined_fields = $this->setPredefinedFields($tt, $options, static::getDefaultValues());
        $this->initForm($this->fields['id'], $options);

        // Kyndryl Customization Start - Profile Access #543 - Change Template
        $canforce_template = false;
        
        if($this->getType() == 'Change') {
            $canforce_template = (bool) Session::haveRight(Change::$rightname, Change::FORCETEMPLATE);
        }
        // Kyndryl Customization Start - 2129745 - Template field for Problem/Ticket
        else if ($this->getType() == 'Problem') {
            $canforce_template = (bool) Session::haveRight(Problem::$rightname, Problem::FORCETEMPLATE);
        }
        // Kyndryl Customization End - 2129745 - Template field for Problem/Ticket

        // Kyndryl Customization End - Profile Access #543 - Change Template

        // Kyndryl Customization Start - DropDown Values AssignedTO and AssginmentGROUP
        $existings = $this->getActorsForType(CommonITILActor::ASSIGN);
        // pass assign data to custom fields created for assign section
        $assign_data = array();
        $_SESSION['glpigroupusers'] = '';
        for($i=0; $i<count($existings); $i++ ){
            if($existings[$i]['itemtype'] == "User" ) {
                $assign_data['assignedto'] = $existings[$i]['items_id'];
            }
            if($existings[$i]['itemtype'] == "Group" ) {
                $assign_data['assignedgroup'] = $existings[$i]['items_id'];
                $_SESSION['glpigroupusers'] = $existings[$i]['items_id'];
            }
        }

        //Kyndryl Customization Start - Template Copy Issue
        if (isset($options['_predefined_fields']['_groups_id_assign']) && !empty($options['_predefined_fields'])) {
            $assign_data['assignedgroup'] = $options['_predefined_fields']['_groups_id_assign'];
        }
        if (isset($options['_predefined_fields']['_users_id_assign']) && !empty($options['_predefined_fields'])) {
            $assign_data['assignedto'] = $options['_predefined_fields']['_users_id_assign'];
        }
         //Kyndryl Customization End - Template Copy Issue
        // Kyndryl Customization End - DropDown Values AssignedTO and Assginment GROUP

        // Kyndryl Customization Start - Requested by
        //Get Requester
        $existings_requester = $this->getActorsForType(CommonITILActor::REQUESTER);
        for($i=0; $i<count($existings_requester); $i++ ){
            if($existings_requester[$i]['itemtype'] == "User" ) {
                $assign_data['requester_id'] = $existings_requester[$i]['items_id'];
            }
        }
        // Kyndryl Customization End - Request By
        ...
        TemplateRenderer::getInstance()->display('components/itilobject/layout.html.twig', [
            ...
            'params'                  => $options,
            'assigned_data'           => $assign_data, // Kyndyrl Custom code start - AssignedTO data retrived from twig
            ...
            // Kyndryl Customization Start - Profile Access #543 - Change Template
            'canforce_template'       => $canforce_template,
            // Kyndryl Customization End - Profile Access #543 - Change Template
            // Kyndryl Customization Start - Profile value send through template
            'profile_id'              => $_SESSION['glpiactiveprofile']['id'],
            'kyn_user_id'             => $_SESSION['glpiID']
            // Kyndryl Customization End - Profile value send through template
        ]);
    ...
    protected function setPredefinedFields(ITILTemplate $tt, array &$options, array $default_values): array
    {
        ...
        if ($this->isNewItem()) {
            if (isset($tt->predefined) && count($tt->predefined)) {
                ...
                // Kyndryl Customization Start - IPC task tab
                foreach ($tt->predefined_tasks as $predeffield => $predefvalue) {
                    $options[$predeffield]           = $predefvalue;
                    $predefined_fields[$predeffield] = $predefvalue;
                    $this->fields[$predeffield]      = $predefvalue;
                }
                // Kyndryl Customization End - IPC task tab
       ...
       // First get ticket template associated : entity and type/category
        ...
        if (
            ...
            if (in_array($this->fields['status'], $this->getClosedStatusArray())) {
                ...
                // Kyndryl Customization Start - Reopen Count Incremented
                $allowed_fields[] = 'kyn_reopen_count';
                // Kyndryl Customization End - Reopen Count Incremented
    ...
    public function post_updateItem($history = true)
    {
        ...
        // Kyndryl Customization Start - Revoke Validation
        $this->manageValidationDelete($this->input);
        $this->manageAssessmsntDelete($this->input);
        $this->createProblemTicket($this->input);   // Kyndryl Customization Start -  create problem ticket #2022419
        // Kyndryl Customization End - Revoke Validation

         // Kyndryl Customization -  #ADO 2073394 - Modification in existing Change Approval and Flow - Start
         if(isset($this->input['status']) && $this->oldvalues['status'] != $this->input['status'] && in_array($this->input['status'], $this->getClosedStatusArray())) {
            $this->cancelPendingTasks();
        // Kyndryl Customization -  #ADO 2073394 - Modification in existing Change Approval and Flow - End
         }
    ...
    public function pre_updateInDB()
    {
       ...
       // Check dates change interval due to the fact that second are not displayed in form
        ...
        if (isset($this->input["status"])) {
            if (
                ...
                if (!in_array("solvedate", $this->updates)){ // Kyndryl Customization Start/End - API solve date is stored
            ...
            if (
                ...
                if (!in_array("closedate", $this->updates)){ // Kyndryl Customization Start/End - API solve date is stored
       ...
       // Manage come back to waiting state
        if (
           ...
           // Compute ticket waiting time use calendar if exists
           // Using calendar
           // Kyndryl Customization Start - SLA Calculation as per entity timezone  
            $ticketobj = new Ticket();
            $timezones = $ticketobj->getSLATimeZone($this->fields['entities_id']);
            $currenttime = $ticketobj->convertTimezone($_SESSION["glpi_currenttime"], $timezones['usertimezone'], $timezones['entitytimezone']);
            $begin_date = $ticketobj->convertTimezone($this->fields['begin_waiting_date'], $timezones['usertimezone'], $timezones['entitytimezone']); 
           // Kyndryl Customization End - SLA Calculation as per entity timezone  
            if (
                ($calendars_id > 0)
                && $calendar->getFromDB($calendars_id)
            ) {
                $delay_time = $calendar->getActiveTimeBetween(
                    //$this->fields['begin_waiting_date'], // Kyndryl Comment  IN/OUT - SLA Calculation as per entity timezone  
                    $begin_date,    // Kyndryl Customization Start/End - SLA Calculation as per entity timezone  
                    // $_SESSION["glpi_currenttime"] // Kyndryl Comment - SLA Calculation as per entity timezone  
                    $currenttime    // Kyndryl Customization Start/End - SLA Calculation as per entity timezone  
                );
            ...
            if (isset($this->fields['slas_id_ttr']) && ($this->fields['slas_id_ttr'] > 0)) {
                ...
                if ($sla->getFromDB($this->fields['slas_id_ttr'])) {
                    ...
                    $delay_time_sla  = $sla->getActiveTimeBetween(
                         // $this->fields['begin_waiting_date'],    // Kyndryl Comment - SLA Calculation as per entity timezone  
                         $begin_date,    // Kyndryl Customization Start/End - SLA Calculation as per entity timezone  
                         // $_SESSION["glpi_currenttime"]   // Kyndryl Comment - SLA Calculation as per entity timezone  
                         $currenttime    // Kyndryl Customization Start/End - SLA Calculation as per entity timezone  
                    );
               ...
               // Compute new time_to_resolve
                $this->updates[]                 = "time_to_resolve";
                 // Kyndryl Customization Start - SLA Calculation as per entity timezone  
                $create_date = $ticketobj->convertTimezone($this->fields['date'], $timezones['usertimezone'], $timezones['entitytimezone']);
                Toolbox::logInFile("output-error", $this->fields["sla_waiting_duration"] . "\n", true);
                 // Kyndryl Customization End - SLA Calculation as per entity timezone  
                $this->fields['time_to_resolve'] = $sla->computeDate(
                     // $this->fields['date'],    // Kyndryl Comment - SLA Calculation as per entity timezone  
                     $create_date,   // Kyndryl Customization Start/End - SLA Calculation as per entity timezone  
                    $this->fields["sla_waiting_duration"]
                );
                // Kyndryl Customization Start - SLA Calculation as per entity timezone  
                $this->fields["time_to_resolve"] = $ticketobj->convertTimezone($this->fields["time_to_resolve"], $timezones['entitytimezone'], $timezones['usertimezone']);
                // Kyndryl Customization End - SLA Calculation as per entity timezone  
    ...
    public function computeDefaultValuesForAdd($input)
    {
        ...
        if (!isset($input["date"]) || empty($input["date"]) || $input["date"] == 'NULL') {
         ...
         /** Kyndryl Customization Start - 
         *  Setting Default value as NULL for DateTime fields type
         */
        if (!isset($input["kyn_planned_start_date"]) || empty($input["kyn_planned_start_date"]) || $input["kyn_planned_start_date"] == 'NULL') {
            $input["kyn_planned_start_date"] = NULL;
        }
        if (!isset($input["kyn_planned_end_date"]) || empty($input["kyn_planned_end_date"]) || $input["kyn_planned_end_date"] == 'NULL') {
            $input["kyn_planned_end_date"] = NULL;
        }
        if (!isset($input["kyn_actual_start_date"]) || empty($input["kyn_actual_start_date"]) || $input["kyn_actual_start_date"] == 'NULL') {
            $input["kyn_actual_start_date"] = NULL;
        }
        if (!isset($input["kyn_outage_start_date"]) || empty($input["kyn_outage_start_date"]) || $input["kyn_outage_start_date"] == 'NULL') {
            $input["kyn_outage_start_date"] = NULL;
        }
        if (!isset($input["kyn_outage_end_date"]) || empty($input["kyn_outage_end_date"]) || $input["kyn_outage_end_date"] == 'NULL') {
            $input["kyn_outage_end_date"] = NULL;
        }
        if (!isset($input["kyn_cab_datetime"]) || empty($input["kyn_cab_datetime"]) || $input["kyn_cab_datetime"] == 'NULL') {
            $input["kyn_cab_datetime"] = NULL;
        }
        if (!isset($input["kyn_due_date"]) || empty($input["kyn_due_date"]) || $input["kyn_due_date"] == 'NULL') {
            $input["kyn_due_date"] = NULL;
        }

         /** Kyndryl Customization End -
          * Setting Default value as NULL for DateTime fields type
         */
    ...
    public function post_clone($source, $history)
    {
        ...
        if (isset($source->fields['users_id_lastupdater'])) {
          ...
          // Kyndryl Customization Start - clone requirement
        /* if (isset($source->fields['status'])) {
            $update['status'] = $source->fields['status'];
        } */

        if(get_class($this) == 'Change'){
            $item_id = 'changes_id';
            $taskitemtype = get_class($this).'Task';
            $itemtype = 'Change';
            $actorclass = 'Change_User';     //Kyndryl Customization Start/End - Clone action is not available on Closed records
        }
        if(get_class($this) == 'Ticket'){
            $item_id = 'tickets_id';
            $taskitemtype = get_class($this).'Task';
            $itemtype = 'Ticket';
            $actorclass = 'Ticket_User';     //Kyndryl Customization Start/End - Clone action is not available on Closed records
        }
        if(get_class($this) == 'Problem'){
            $item_id = 'problems_id';
            $taskitemtype = get_class($this).'Task';
            $itemtype = 'Problem';
            $actorclass = 'Problem_User';    //Kyndryl Customization Start/End - Clone action is not available on Closed records

        }
        // For Task and task_document clone 
        if($item_id){
            $itemtaskobj = new $taskitemtype;
            $sourceitemtask = $itemtaskobj->find([
                $item_id    => $source->fields['id'],
            ]);
            foreach($sourceitemtask as $itemtask) {
                $content = (!empty(strip_tags(Sanitizer::decodeHtmlSpecialChars($itemtask['content'])))) ? strip_tags(Sanitizer::decodeHtmlSpecialChars($itemtask['content'])) : NULL;
             // Kyndryl Customization Start - 2164426 - add outage field
             if(get_class($this) == 'Change'){
                $itemtaskid = $itemtaskobj->add([
                    $item_id            => $this->getID(),
                    'taskcategories_id' => $itemtask['taskcategories_id'],
                    'state'             => 0,
                    'users_id_tech'     => $itemtask['users_id_tech'],
                    'groups_id_tech'    => $itemtask['groups_id_tech'],
                    'timeline_position' => $itemtask['timeline_position'],
                    'kyn_sequence'      => $itemtask['kyn_sequence'],
                    'kyn_name'          => $itemtask['kyn_name'],//Kyndryl Customization Start/End - New Field 
                    'kyn_on_hold_reasontext'          => $itemtask['kyn_on_hold_reasontext'],//Kyndryl Customization Start/End - New Field 
                    'kyn_on_hold'          => $itemtask['kyn_on_hold'],//Kyndryl Customization Start/End - New Field 
                    'kyn_outage'        => $itemtask['kyn_outage'], // Kyndryl Customization - 2164426 - add outage field
                    //'kyn_predecessor'   => $itemtask['kyn_predecessor'],
                    'content'           => $content,
                ]);
            }
            else{
            ...
            // Kyndryl Customization End - 2164426 - add outage field
                // $documentitemobj = new Document_Item(); /*Kyndryl Customization Start-2162899 - Clone Requirement */
                // $documentitems = $documentitemobj->find([
                //     'items_id'        => $itemtask['id'],
                //     'itemtype'        => $taskitemtype,
                // ]);/*Kyndryl Customization End -2162899 - Clone Requirement */
                if(!empty($documentitems)){
                    foreach($documentitems as $documentitem) {
                        $documentitemobj->add([
                            'documents_id'  =>  $documentitem['documents_id'],
                            'items_id'      =>  $itemtaskid,
                            'itemtype'      =>  $documentitem['itemtype'],
                            'entities_id'   =>  $documentitem['entities_id'],
                            'is_recursive ' =>  $documentitem['is_recursive'],
                        ]);
                    }
                }
                // Kyndryl Customization Start - 2164410 - Allow Tasks also for clone
                if (get_class($this) == 'Change') {
                    $kyn_changetask_item_obj = new ChangeTask_Item();
                    $kyn_changetask_found_item = $kyn_changetask_item_obj->find([
                        'changetasks_id'        => $itemtask['id'],
                    ]);
                    
                    if(!empty($kyn_changetask_found_item)){
                        foreach($kyn_changetask_found_item as $kyn_changetask_item) {
                            $kyn_changetask_item_obj->add([
                                'changetasks_id'      =>  $itemtaskid,
                                'itemtype'      =>  $kyn_changetask_item['itemtype'],
                                'items_id'      =>  $kyn_changetask_item['items_id'],
                            ]);
                        }
                    }
                }
                // Kyndryl Customization End- 2164410 - Allow Tasks also for clone
            }
        }        

        // Clone Followup attachments only ad documents
        if($itemtype){
            $followupobj = new ITILFollowup();
            $sourcefollowups = $followupobj->find([
                'items_id'        => $source->fields['id'],
                'itemtype'        => get_class($this),
            ]);
            foreach($sourcefollowups as $followup) {
                $documentitemobj = new Document_Item();
                $documentitems = $documentitemobj->find([
        ...
        //Kyndryl Customization Start - Clone action is not available on Closed records
        if($actorclass){
            $actorobj = new $actorclass;    // // Kyndryl Customization Start - 2053741 - clone field
            $actorobj->add([
                $item_id => $this->getID(),
          ...
          //Kyndryl Customization End - Clone action is not available on Closed records

        // Kyndryl Customization End - clone requirement
        $DB->update(
            $this->getTable(),
            $update,
            ['id' => $this->getID()]
    ...
    public function getCloneRelations(): array
    {
        ...
        // Kyndryl Customization Start - Clone action is not available on Closed records 2053741
        //if (is_a($this->userlinkclass, CommonITILActor::class, true)) {
        //    $relations[] = $this->userlinkclass;
        //}
        // Kyndryl Customization End - Clone action is not available on Closed records 2053741
    ...
    public static function getSpecificValueToDisplay($field, $values, array $options = [])
    {
        ...
        switch ($field) {
            ...
            //Kyndryl Customization Start - Dropdown Values Function
            case 'kyn_reported_priority':
                return static::getReportedPriority($values[$field]);

            case 'kyn_on_hold_reason':
                return static::getOnHoldReason($values[$field]);
            
            case 'kyn_resolution_code':
                return static::getResolutionCode($values[$field]);

            case 'kyn_template':
                return static::getTemplate($values[$field]);

            case 'kyn_sub_state':
                return static::getSubState($values[$field]);

            case 'kyn_failure_code':
                return static::getFailureCode($values[$field]);

            // Kyndryl Customization Start - Two New Fields For Request
            case 'kyn_implementation_code':
                return static::getImplementationCode($values[$field]);
            // Kyndryl Customization End - Two New Fields For Request

            case 'kyn_risk ':
                return static::getRisk($values[$field]);

            case 'kyn_close_code':
                return static::getCloseCode($values[$field]);

            case 'kyn_failure_probability':
                return static::getFailureProbability($values[$field]);

            case 'kyn_change_type':
                return static::getChangeType($values[$field]);
            //Kyndryl Customization End - Dropdown Values Function
    ...
    public static function getSpecificValueToSelect($field, $name = '', $values = '', array $options = [])
    {
        ...
        switch ($field) {
            ...
            case 'kyn_reported_priority':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownReportedPriority($options);

            case 'kyn_on_hold_reason':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownOnHoldReason($options);

            case 'kyn_resolution_code':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownResolutionCode($options);
            
            case 'kyn_template':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownTemplate($options);

            case 'kyn_risk':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownRisk($options);

            case 'kyn_close_code':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownCloseCode($options);

            case 'kyn_failure_probability':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownFailureProbability($options);

            case 'kyn_change_type':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownChangeType($options);

            case 'kyn_sub_state':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownSubState($options);

            case 'kyn_failure_code':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownFailureCode($options);
            
            // Kyndryl Customization Start - Two New Fields For Request
            case 'kyn_implementation_code':
                $options['name']  = $name;
                $options['value'] = $values[$field];
                return static::dropdownImplementationCode($options);
            // Kyndryl Customization End - Two New Fields For Request
    ...
    public function getSearchOptionsMain()
    {
        ...
        $tab[] = [
            ...
            'name'               => __('ID'),
            'massiveaction'      => false,
            'searchtype'         => ['equals', 'notequals','isoneof'],
            'datatype'           => 'number'
        ];
        ...
        # Kyndryl Customization Start - Search Options For Additional Fields
        # Kyndryl Customization Start - Single Line Text Fields
        # Kyndryl Customization Start - Correlation Display

        $tab[] = [
            'id'                 => '1001',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_display',
            'name'               => __('Correlation Display'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation Display
        # Kyndryl Customization Start - Correlation ID

        $tab[] = [
            'id'                 => '1002',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_id',
            'name'               => __('Correlation ID'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation ID

        # Kyndryl Customization Start - Correlation Display 1

        $tab[] = [
            'id'                 => '1061',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_display1',
            'name'               => __('Correlation Display 1'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation Display 1
        # Kyndryl Customization Start - Correlation ID 1

        $tab[] = [
            'id'                 => '1057',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_id1',
            'name'               => __('Correlation ID 1'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation ID 1

        # Kyndryl Customization Start - Correlation Display 2

        $tab[] = [
            'id'                 => '1062',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_display2',
            'name'               => __('Correlation Display 2'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation Display 2
        # Kyndryl Customization Start - Correlation ID 2

        $tab[] = [
            'id'                 => '1058',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_id2',
            'name'               => __('Correlation ID 2'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation ID 2

        # Kyndryl Customization Start - Correlation Display 3

        $tab[] = [
            'id'                 => '1063',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_display3',
            'name'               => __('Correlation Display 3'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation Display 3
        # Kyndryl Customization Start - Correlation ID 3

        $tab[] = [
            'id'                 => '1059',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_id3',
            'name'               => __('Correlation ID 3'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation ID 3

        # Kyndryl Customization Start - Correlation Display 4

        $tab[] = [
            'id'                 => '1064',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_display4',
            'name'               => __('Correlation Display 4'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation Display 4
        # Kyndryl Customization Start - Correlation ID 4

        $tab[] = [
            'id'                 => '1060',
            'table'              => $this->getTable(),
            'field'              => 'kyn_correlation_id4',
            'name'               => __('Correlation ID 4'),
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Correlation ID 4

        # Kyndryl Customization End - Single Line Text Fields
        # Kyndryl Customization Start - Time Fields Text Fields
        # Kyndryl Customization Start - Actual Start Date

        $tab[] = [
            'id'                 => '1018',
            'table'              => $this->getTable(),
            'field'              => 'kyn_actual_start_date',
            'name'               => __('Actual Start Date'),
            'datatype'           => 'datetime',
            'massiveaction'      => false
        ];

        # Kyndryl Customization End - Actual Start Date
        # Kyndryl Customization End - Time Fields Text Fields
        # Kyndryl Customization Start - Dropdown Fields
        # Kyndryl Customization Start - Template
        ...
        # Kyndryl Customization End - Template
        # Kyndryl Customization End - Dropdown Fields
        # Kyndryl Customization End - Search Options For Additional Fields
    ...
    public function getSearchOptionsActors()
    {
        ...
        //zntest
       /* $newtab = [
            'id'                 => '4899', // Also in Ticket_User::post_addItem() and Log::getHistoryData()
            'table'              => 'glpi_users',
            'field'              => 'id',
            'datatype'           => 'dropdown',
            'right'              => 'all',
        ...
        if (
           ...
           // Using calendar
            if (
                ...
                $timezones = $ticketobj->getSLATimeZone($this->fields['entities_id']);
                $create_date = $ticketobj->convertTimezone($this->fields['date'], $timezones['usertimezone'], $timezones['entitytimezone']);
                if($this->fields['solvedate'] != 'NULL'){                    
                    $solve_date = $ticketobj->convertTimezone($this->fields['solvedate'], $timezones['usertimezone'], $timezones['entitytimezone']);
                } else {
                    $solve_date = $this->fields['solvedate'];
                }
                // Kyndryl Customization End - SLA Calculation as per entity timezone
                return max(0, $calendar->getActiveTimeBetween(
                    // $this->fields['date'],   // Kyndryl Comment - SLA Calculation as per entity timezone
                    $create_date,   // Kyndryl Customization Start/End - SLA Calculation as per entity timezone
                    // $this->fields['solvedate']   // Kyndryl Comment - SLA Calculation as per entity timezone
                    $solve_date // Kyndryl Customization Start/End - SLA Calculation as per entity timezone
                )
        ...
        if (
           ...
           // Using calendar
            if (
                ...
                $timezones = $ticketobj->getSLATimeZone($this->fields['entities_id']);
                $create_date = $ticketobj->convertTimezone($this->fields['date'], $timezones['usertimezone'], $timezones['entitytimezone']);
                if($this->fields['closedate'] != 'NULL'){                    
                    $close_date = $ticketobj->convertTimezone($this->fields['closedate'], $timezones['usertimezone'], $timezones['entitytimezone']);
                } else {
                    $close_date = $this->fields['closedate'];
                }
                // Kyndryl Customization End - SLA Calculation as per entity timezone
                return max(0, $calendar->getActiveTimeBetween(
                    // $this->fields['date'],  // Kyndryl Comment - SLA Calculation as per entity timezone
                    $create_date,   // Kyndryl Customization Start/End - SLA Calculation as per entity timezone
                    // $this->fields['closedate']  // Kyndryl Comment - SLA Calculation as per entity timezone
                    $close_date     // Kyndryl Customization Start/End - SLA Calculation as per entity timezone
                )
                                                             - $this->fields["waiting_duration"]);
            }
           // Not calendar defined
    ...
    public function getTimelineItemtypes(): array
    {
        ...
        $canadd_solution = $obj_type::canUpdate() && $this->canSolve() && !in_array($this->fields["status"], $solved_statuses, true);
         ...
         // Kyndryl Customization - Issue #681 - ALL: IPC , Request and Task can be editable by only member of assignment group - Start
         /*Kyndryl Customization code for Answer button Starts - ADO #2199879
         if(!$this->canUpdateItem()) {
            $canadd_fup = false;
            $canadd_document = false;
            $canadd_solution = false;
          }
            Kyndryl Customization code for Answer button Ends - ADO #2199879 */
        // Kyndryl Customization - Issue #681 - ALL: IPC , Request and Task can be editable by only member of assignment group - End

        // Kyndryl Customization Start - Add A Solution - two new fields #177
        if (!in_array($this->fields["status"], [self::ASSIGNED, self::PLANNED])) {
            $canadd_solution = false;
        }
        // Kyndryl Customization End - Add A Solution - two new fields #177

        // Kyndryl Customization Start - Problem Redesign
        if($obj_type == 'Problem') {
            if ($this->fields["kyn_sub_state"] != 2) {
                $canadd_solution = false;
            }
            $problemTaskObj = new ProblemTask();
            $allTasks = $problemTaskObj->find(['problems_id' => $this->fields['id']]);
            $completedTasks = $problemTaskObj->find(['problems_id' => $this->fields['id'], 'state' => [
                Planning::DONE, Planning::CANCELLED, Planning::NOT_REQUIRED, Planning::FAILED
            ]]);
            if($allTasks > 0 && $allTasks != $completedTasks) {
                $canadd_solution = false;    
            }
        }
        // Kyndryl Customization End - Problem Redesign

        // Kyndryl Customization Start - Ticket Redesign
        if($obj_type == 'Ticket') {
            $ticketTaskObj = new TicketTask();
            $allTasks = $ticketTaskObj->find(['tickets_id' => $this->fields['id']]);
            $completedTasks = $ticketTaskObj->find(['tickets_id' => $this->fields['id'], 'state' => [
                Planning::DONE, Planning::CANCELLED, Planning::NOT_REQUIRED, Planning::FAILED
            ]]);
        ...
        // Kyndryl Customization End - Ticket Redesign
        ...
        // Kyndryl Customization Start - Add A validaton removed from IPC
        ...
        // Kyndryl Customization Start - Change Task Tab 
        if($obj_type != 'Change') {
        // Kyndryl Customization End - Change Task Tab
            /* Kyndryl Comment In
             $itemtypes['task'] = [
                'type'          => 'ITILTask',
                'class'         => $task_class,
       ...
       //checks rights
        ...
        if (!$params['check_view_rights'] || $followup_obj->canview()) {
            ...
            foreach ($followups as $followups_id => $followup_row) {
                ...
                if (!$params['check_view_rights'] || $followup->canViewItem()) {
                    $followup_row['can_edit'] = ($followup->fields["users_id"] == Session::getLoginUserID());
            ...
            if ($this->getType() == 'Problem'){
                $timeline["ITILSolution_" . $solution_item['id'] ] = [
                    ...
                    'item'     => [
                        ...
                        // Kyndryl Customization Start  - Add A Solution #177
                        'kyn_resolution_code'=> self::getResolutionCode($solution_item['kyn_resolution_code']),
                        'kyn_fix_notes'      => $solution_item['kyn_fix_notes'],
                        'kyn_failure_code'   => self::getFailureCode($solution_item['kyn_failure_code']),
                        // Kyndryl Customization End  - Add A Solution #177
                ...
                $timeline["ITILSolution_" . $solution_item['id'] ] = [
                    ...
                    'item'     => [
                        ...
                        'kyn_fix_notes'     => $solution_item['kyn_fix_notes'],
                        'kyn_failure_code'   => self::getFailureCode($solution_item['kyn_failure_code']),
                        'kyn_cause_notes'   => $solution_item['kyn_cause_notes'],
                        // Kyndryl Customization Start - Two New Fields For Request
                        'kyn_implementation_notes'     => $solution_item['kyn_implementation_notes'],
                        'kyn_implementation_code'   => self::getImplementationCode($solution_item['kyn_implementation_code']),
                        // Kyndryl Customization End - Two New Fields For Request
                        // Kyndryl Customization End  - Add A Solution #177
                        'can_edit'           => $objType::canUpdate() && $this->canSolve(),
                        'timeline_position'  => self::TIMELINE_RIGHT,
        ...
        if (
            ...
            foreach ($validations as $validations_id => $validation_row) {
                ...
                // Kyndryl Customization Start - Change: Admin and Super Admin of the entity can be able to approve the change on behalf of the Users
                $is_super_admin = $validation_row['users_id_validate'] != Session::getLoginUserID() && Profile::isCurrentProfileSuperAdmin();
                
                $cananswer = (($validation_row['users_id_validate'] === Session::getLoginUserID() || $is_super_admin) &&
                ...
                $timeline[$request_key] = [
                    ...
                    'item' => [
                        ...
                        'can_edit'  => $canedit,
                       // Kyndryl Customization - Restrict Validation on Closed Items - Start
                       'can_answer'   => !in_array($this->fields['status'], array_merge($this->getClosedStatusArray(), $this->getSolvedStatusArray())) && $cananswer,
                       // Kyndryl Customization - Restrict Validation on Closed Items - End
                ...
                // Kyndryl Customization End - Change: Admin and Super Admin of the entity can be able to approve the change on behalf of the Users
    ...
    public function getForbiddenSingleMassiveActions()
    {
        ...
        // Kyndyrl Customization start - remove add and cancle on solved and closed
        if($this->getType() =='Problem'){
            if($this->fields['status'] == CommonITILObject::SOLVED || $this->fields['status'] == CommonITILObject::CLOSED) {
               // $excluded[] = 'Document_Item:add';
                $excluded[] = 'Problem:cancel_problem';
            }
        }
        // Kyndyrl Customization start - remove add and cancle on solved and closed
        return $excluded;
    ...
    public function handleTaskTemplateInput()
    {
       // Check input is valid
        ...
        $itilTasksArray = [];
        foreach ($this->input['_tasktemplates_id'] as $tasktemplates_id) {
            $tasktemplates_id = explode('_',$tasktemplates_id);
            $itilTasksArray[$tasktemplates_id[0]] = $tasktemplates_id;
            $taskID = $itiltask->add([
                '_tasktemplates_id'           => $tasktemplates_id[0],
                $this->getForeignKeyField()   => $this->fields['id'],
                'date'                        => $this->fields['date'],
                '_disablenotif'               => true,
                // Kyndryl Customization Start - Task Target Start End and Group User Fix
                'users_id_tech'               => isset($this->input['_users_id_assign'][0]) ? $this->input['_users_id_assign'][0] : 0,
                'groups_id_tech'              => isset($this->input['_groups_id_assign'][0]) ? $this->input['_groups_id_assign'][0] : null,
                'kyn_target_begin'            => $this->input['kyn_planned_start_date'],
                'kyn_target_end'              => $this->input['kyn_planned_end_date'],
                // Kyndryl Customization End - Task Target Start End and Group User Fix 
            ]);
            array_push($itilTasksArray[$tasktemplates_id[0]], $taskID);  
        }
        
        foreach ($itilTasksArray as $tasktemplates_id) {
            $itiltask   = new $itiltask_class();

            $itiltype = $this->getType() . 'Task';
            
            $predecessorTaskObj = new $itiltype();
            $predecessorTasks = $predecessorTaskObj->find([
                'tasktemplates_id' => $tasktemplates_id[2],
                $this->getForeignKeyField()   => $this->fields['id'],
            ]);

            $predecessorTask = array_values($predecessorTasks)[0];
            
            $itiltask->update([
                'id' => $tasktemplates_id[3],
                'kyn_sequence'                => $tasktemplates_id[1],
                'kyn_predecessor'             => isset($predecessorTask) ? ($predecessorTask['id'] ?? null) : null,
            ]);
        ...
        if (isset($input["_add_validation"])) {
            ...
            foreach ($input["_add_validation"] as $key => $value) {
                switch ($value) {
                    ...
                    // Kyndryl Customization Start - Business rule approvals
                    case 'assignment_group':
                        if (
                            isset($input['_groups_id_assign'])
                            && $input['_groups_id_assign']
                        ) {
                            // Kyndryl Customization Start - Access check and filtration #156
                            $change_validation_right = 'validate';                            
                            $opt = [
                                'groups_id' => $input['_groups_id_assign'],
                                'right'     => $change_validation_right,
                                'entity'    => $entid
                            ];         
                            $users = $validation->getGroupUserHaveRights($opt);
                            // Kyndryl Customization End - Access check and filtration #156
                            foreach ($users as $data) {
                                $validations_to_send[] = $data['id'];
                                $validation_group[$data['id']] = reset($input['_groups_id_assign']);
                                $validation_requestercomment[$data['id']] = 'Assignment Group';
                            }
                        }
                        break;

                        case 'ci_owner_groups':
                            if (
                                isset($input['id'])
                                && $input['id']
                            ) {
                                $config_item = new Change_Item();
                                $config_item_data = $config_item->find(['changes_id' => $input['id']]);
                                foreach($config_item_data as $key ){        
                                    $itemtype = new $key['itemtype'];
                                    $item_data = $itemtype->find(['id' => $key['items_id']]);  
                                    $item_data2 = reset($item_data);     
                                    // Kyndryl Customization Start - Access check and filtration #156
                                    $change_validation_right = 'validate';                            
                                    $opt = [
                                        'groups_id' => $item_data2['groups_id_tech'],
                                        'right'     => $change_validation_right,
                                        'entity'    => $entid
                                    ];         
                                    $users = $validation->getGroupUserHaveRights($opt);
                                    ...
                                    foreach ($users as $data) {
                                        $validations_to_send[] = $data['id'];
                                        $validation_group[$data['id']] = $item_data2['groups_id_tech'];
                                        $validation_requestercomment[$data['id']] = 'CI Owner Group';
                                    }  
                                }
                            }
                            break;
                    ...
                    case 'cab_group':
                        if (
                            ...
                            $cabgroup = Entity::getUsedConfig('kyn_cab_group', $entid, '', 1);
                            // Kyndryl Customization Start - Access check and filtration #156
                            $change_validation_right = 'validate';                            
                            $opt = [
                                'groups_id' => $cabgroup,
                            ...
                            foreach ($users as $data) {
                                $validations_to_send[] = $data['id'];
                                $validation_group[$data['id']] = $cabgroup;
                                $validation_requestercomment[$data['id']] = 'CAB Group Approval';
                            }
                        }
                        break;
                        
                    case 'change_manager_group':
                        if (
                            isset($input['id'])
                            && $input['id']
                        ) {
                            $changemanagergroup = Entity::getUsedConfig('kyn_change_manager_group', $entid, '', 1);         
                            // Kyndryl Customization Start - Access check and filtration #156
                            $change_validation_right = 'validate';                            
                            $opt = [
                                'groups_id' => $changemanagergroup,
                                'right'     => $change_validation_right,
                                'entity'    => $entid
                            ];         
                            $users = $validation->getGroupUserHaveRights($opt);
                            ...
                            foreach ($users as $data) {
                                $validations_to_send[] = $data['id'];
                                $validation_group[$data['id']] = $changemanagergroup;
                                $validation_requestercomment[$data['id']] = 'Change Manager Group Approval';
                            }
                        }
                        break;

                    // Kyndryl Customization Start - Manual approver need to trigger when change moves to Approval State
                    case 'manual_queued_approval':
                        if (
                            isset($input['id'])
                            && $input['id']
                            ) {
                                ...
                                $change->getFromDB($input['id']);
                                
                                $users = importArrayFromDB($change->fields['queued_approvals']);
                                
                                foreach ($users as $user_id) {
                                    if(!in_array($user_id, $validations_to_send)) {
                                        $validations_to_send[] = $user_id;
                                        $validation_group[$user_id] = 'M/A';
                                        $validation_requestercomment[$user_id] = 'Manual Approval';
                                    }
                                }
                            }
                        break;
                    // Kyndryl Customization End - Manual approver need to trigger when change moves to Approval State
                    
                    // Kyndryl Customization Start - Manual Group Approval
                    case 'manual_queued_group_approval':
                        if (
                            isset($input['id'])
                            && $input['id']
                            ) {
                                ...
                                $groups = importArrayFromDB($change->fields['queued_group_approvals']);
                                ...
                                foreach ($groups as $groups_id) {
                                    ...
                                    foreach ($data_users as $user) {
                                        ...
                                        // Kyndryl Customization - Change: Missing comment section - Start
                                        if(isset($validation_group[$user_id])) {
                                            if(!is_array($validation_group[$user_id])) {
                                                $validation_group[$user_id] = [$validation_group[$user_id], $groups_id];

                                            } else if(is_array($validation_group[$user_id])) {
                                                
                                                array_push($validation_group[$user_id], $groups_id);
                                            }
                                        } else {
                                            $validation_group[$user_id] = $groups_id;
                                        }
                                        // Kyndryl Customization - Change: Missing comment section - End
                                        $validation_requestercomment[$user_id] = 'Manual Group Approval';
                                    }
                                }
                            }
                        break;
                    // Kyndryl Customization End - Manual Group Approval

                    case 'change_owner':
                        if ($input['id']) {
                            $changeuserobj = new Change_User();
                            $change_owner = $changeuserobj->find([
                                'changes_id' => $input['id'],
                                'type'       => 2
                            ]);
                            $validations_to_send[] = reset($change_owner)['users_id'];
                            $validation_requestercomment[reset($change_owner)['users_id']] = 'Change Owner Approval';
                        }
                        break;
                    // Kyndryl Customization Start - Business rule approvals
            ...
            if (count($validations_to_send)) {
                ...
                if (
                    ...
                    foreach ($validations_to_send as $user) {
                        ...
                        if (!$validation->alreadyExists($values[$self_fk], $user) || true) {  //Kyndryl Customization Start/End - Double Approvals
                            $values["users_id_validate"] = $user;
                            // Kyndryl Customization Start - add if condition to add group id in validation
                             // Kyndryl Customization - Change: Missing comment section - Start
                             if($validation_group[$user]){
                                if(is_array($validation_group[$user])) {
                                    $values["groups_id"] = array_shift($validation_group[$user]);
                                } else {
                                    $values["groups_id"] = $validation_group[$user];
                                }
                            }
                            $values["comment_submission"] = $validation_requestercomment[$user];
                            
                            if(str_contains($values["comment_submission"], 'Manual Group Approval')) {
                                $values["comment_submission"] = sprintf('Manual Group Approval - %s', Group::getFriendlyNameById($values["groups_id"]));
        ...
        foreach ($actor_types as $actor_type) {
            ...
            foreach ($added as $actor) {
                ...
                if (
                    ...
                    // Kyndryl Customization Start - Processing state only when user is attached to the Item

                    if($actor['itemtype']=='User'){ //if user assigned update status to assigned.
                        $self->update(
                            [
                                'id'                              => $this->getID(),
                    ...
                    // Kyndryl Customization End - Processing state only when user is attached to the Item
    ...
    // Kyndryl Customization Start - dropdownReportedPriority Choices
    public static function dropdownReportedPriority(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_reported_priority',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getReportedPriority(0);
        $values[2] = static::getReportedPriority(2);
        $values[3] = static::getReportedPriority(3);
        $values[4] = static::getReportedPriority(4);
        $values[5] = static::getReportedPriority(5);


        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getReportedPriority($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_reported_priority', '');

            case 2:
                return _x('kyn_reported_priority', 'Low');

            case 3:
                return _x('kyn_reported_priority', 'Medium');
            
            case 4:
                return _x('kyn_reported_priority', 'High');
            
            case 5:
                return _x('kyn_reported_priority', 'Very High');
    ...
    // Kyndryl Customization End - dropdownReportedPriority Choices

    // Kyndryl Customization Start - dropdownOnHoldReason Choices
    public static function dropdownOnHoldReason(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_on_hold_reason',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getOnHoldReason(0);
        $values[1] = static::getOnHoldReason(1);
        $values[2] = static::getOnHoldReason(2);
        $values[3] = static::getOnHoldReason(3);
        $values[8] = static::getOnHoldReason(8);
        $values[9] = static::getOnHoldReason(9);
        $values[10] = static::getOnHoldReason(10);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getOnHoldReason($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_on_hold_reason', '');

            case 1:
                return _x('kyn_on_hold_reason', 'Awaiting Caller');

            case 2:
                return _x('kyn_on_hold_reason', 'Awaiting Customer');

            case 3:
                return _x('kyn_on_hold_reason', 'Awaiting Vendor');

            case 8:
                return _x('kyn_on_hold_reason', 'SLAHOLD - Customer Unavailable 1st Attempt');

            case 9:
                return _x('kyn_on_hold_reason', 'SLAHOLD - Customer Unavailable 2nd Attempt');

            case 10:
                return _x('kyn_on_hold_reason', 'SLAHOLD - Customer Unavailable 3rd Attempt');
    ...
    // Kyndryl Customization End - dropdownOnHoldReason Choices

    // Kyndryl Customization Start - dropdownResolutionCode Choices
    public static function dropdownResolutionCode(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_resolution_code',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getResolutionCode(0);
        $values[1] = static::getResolutionCode(1);
        $values[2] = static::getResolutionCode(2);
        $values[3] = static::getResolutionCode(3);
        $values[4] = static::getResolutionCode(4);
        $values[5] = static::getResolutionCode(5);
        $values[6] = static::getResolutionCode(6);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getResolutionCode($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_resolution_code', '');

            case 1:
                return _x('kyn_resolution_code', 'Risk Accepted');

            case 2:
                return _x('kyn_resolution_code', 'Fix Applied');
            
            case 3:
                return _x('kyn_resolution_code', 'Duplicate');

            case 4:
                return _x('kyn_resolution_code', 'Canceled');

            case 5:
                return _x('kyn_resolution_code', 'Workaround Accepted');
    
            case 6:
                return _x('kyn_resolution_code', 'Customer Agreed Closure');
    ...
    // Kyndryl Customization End - dropdownResolutionCode Choices

    // Kyndryl Customization Start - dropdownTemplate Choices
    public static function dropdownTemplate(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;
        ...
        $values[0] = static::getTemplate(0);
        $values[1] = static::getTemplate(1);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getTemplate($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_template', '');

            case 1:
                return _x('kyn_template', 'Under Dev');
    ...
    // Kyndryl Customization End - dropdownTemplate Choices

    // Kyndryl Customization Start - dropdownSubState Choices
    public static function dropdownSubState(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;
        ...
        $values[0] = static::getSubState(0);
        $values[1] = static::getSubState(1);
        $values[2] = static::getSubState(2);
        $values[3] = static::getSubState(3);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getSubState($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_sub_state', '');

            case 1:
                return _x('kyn_sub_state', 'Root Cause Analysis');

            case 2:
                return _x('kyn_sub_state', 'Fix In Progress');

            case 3:
                return _x('kyn_sub_state', 'RCA Completed');    
    ...
    // Kyndryl Customization End - dropdownSubState Choices

    // Kyndryl Customization Start - dropdownFailureCode Choices
    public static function dropdownImplementationCode(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_implementation_code',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getImplementationCode(0);
        $values[1] = static::getImplementationCode(1);
        $values[2] = static::getImplementationCode(2);
        $values[3] = static::getImplementationCode(3);
        $values[4] = static::getImplementationCode(4);
        $values[5] = static::getImplementationCode(5);
        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getImplementationCode($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_implementation_code', '');

            case 1:
                return _x('kyn_implementation_code', 'Implemented');

            case 2:
                return _x('kyn_implementation_code', 'Partially Implemented');

            case 3:
                return _x('kyn_implementation_code', 'Implementation Failed');

            case 4:
                return _x('kyn_implementation_code', 'Cancelled');

            case 5:
                return _x('kyn_implementation_code', 'Rejected');
    ...
    // Kyndryl Customization End - dropdownFailureCode Choices

    // Kyndryl Customization Start - dropdownFailureCode Choices
    public static function dropdownFailureCode(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_failure_code',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getFailureCode(0);
        $values[1] = static::getFailureCode(1);
        $values[2] = static::getFailureCode(2);
        $values[3] = static::getFailureCode(3);
        $values[4] = static::getFailureCode(4);
        $values[5] = static::getFailureCode(5);
        $values[6] = static::getFailureCode(6);
        $values[7] = static::getFailureCode(7);
        $values[8] = static::getFailureCode(8);
        $values[9] = static::getFailureCode(9);
        $values[10] = static::getFailureCode(10);
        $values[11] = static::getFailureCode(11);
        $values[12] = static::getFailureCode(12);
        $values[13] = static::getFailureCode(13);
        $values[14] = static::getFailureCode(14);
        $values[15] = static::getFailureCode(15);
        $values[16] = static::getFailureCode(16);
        $values[17] = static::getFailureCode(17);
        $values[18] = static::getFailureCode(18);
        $values[19] = static::getFailureCode(19);
        $values[20] = static::getFailureCode(20);
        $values[21] = static::getFailureCode(21);
        $values[22] = static::getFailureCode(22);
        $values[23] = static::getFailureCode(23);
        $values[24] = static::getFailureCode(24);
        $values[25] = static::getFailureCode(25);
        $values[26] = static::getFailureCode(26);
        $values[27] = static::getFailureCode(27);
        $values[28] = static::getFailureCode(28);
        $values[29] = static::getFailureCode(29);
        $values[30] = static::getFailureCode(30);
        $values[31] = static::getFailureCode(31);
        $values[32] = static::getFailureCode(32);
        $values[33] = static::getFailureCode(33);
        $values[34] = static::getFailureCode(34);
        $values[35] = static::getFailureCode(35);
        $values[36] = static::getFailureCode(36);
        $values[37] = static::getFailureCode(37);
        $values[38] = static::getFailureCode(38);
        $values[39] = static::getFailureCode(39);
        $values[40] = static::getFailureCode(40);
        $values[41] = static::getFailureCode(41);


        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getFailureCode($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_failure_code', '');

            case 1:
                return _x('kyn_failure_code', 'Network');

            case 2:
                return _x('kyn_failure_code', 'Storage');

            case 3:
                return _x('kyn_failure_code', 'Application');

            case 4:
                return _x('kyn_failure_code', 'Customer');

            case 5:
                return _x('kyn_failure_code', 'File System/Dataset');

            case 6:
                return _x('kyn_failure_code', 'Process');

            case 7:
                return _x('kyn_failure_code', 'Telephony');

            case 8:
                return _x('kyn_failure_code', 'Capacity');

            case 9:
                return _x('kyn_failure_code', 'Other');

            case 10:
                return _x('kyn_failure_code', 'User Admin');

            case 11:
                return _x('kyn_failure_code', 'Backup Restore');

            case 12:
                return _x('kyn_failure_code', 'Not a problem');

            case 13:
                return _x('kyn_failure_code', 'Unknown');

            case 14:
                return _x('kyn_failure_code', 'Change');

            case 15:
                return _x('kyn_failure_code', 'Printing');

            case 16:
                return _x('kyn_failure_code', 'Facilities');

            case 17:
                return _x('kyn_failure_code', 'Software');

            case 18:
                return _x('kyn_failure_code', 'Hardware');

            case 19:
                return _x('kyn_failure_code', 'Support');

            case 20:
                return _x('kyn_failure_code', 'Cancel');

            case 21:
                return _x('kyn_failure_code', 'Oversight/Distraction');
    
            case 22:
                return _x('kyn_failure_code', 'Database');
    
            case 23:
                return _x('kyn_failure_code', 'Software Bug');
    
            case 24:
                return _x('kyn_failure_code', 'Patching');
    
            case 25:
                return _x('kyn_failure_code', 'Customer vendor');
    
    
            case 26:
                return _x('kyn_failure_code', 'Environmental');
    
            case 27:
                return _x('kyn_failure_code', 'Procedure not followed');
    
            case 28:
                return _x('kyn_failure_code', 'EOL/EOS');
    
            case 29:
                return _x('kyn_failure_code', 'Kyndryl - 3rd party vendor');
    
            case 30:
                return _x('kyn_failure_code', 'Alerting');
    
            case 31:
                return _x('kyn_failure_code', 'Communication');
    
            case 32:
                return _x('kyn_failure_code', 'No Health Check/housekeeping');
    
            case 33:
                return _x('kyn_failure_code', 'Build/Design');
    
            case 34:
                return _x('kyn_failure_code', 'Wrong procedure');
    
            case 35:
                return _x('kyn_failure_code', 'Power Outage');
    
            case 36:
                return _x('kyn_failure_code', 'Firewall/Port issue');
    
            case 37:
                return _x('kyn_failure_code', 'Cabling/Wrong cabling');
       
            case 38:
                return _x('kyn_failure_code', 'Access/Permission');
       
            case 39:
                return _x('kyn_failure_code', 'Tools');
       
            case 40:
                return _x('kyn_failure_code', 'No Test Environment');
    
            case 41:
                return _x('kyn_failure_code', 'Insufficient memory/disk space');
    ...
    // Kyndryl Customization Start - dropdownRisk Choices
    public static function dropdownRisk(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;
        ...
        $values[0] = static::getRisk(0);
        $values[2] = static::getRisk(2);
        $values[3] = static::getRisk(3);
        $values[4] = static::getRisk(4);
        $values[5] = static::getRisk(5);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getRisk($value)
    {
        ...
        switch ($value) {
            ...
            case 2:
                return _x('kyn_risk', 'Low');

            case 3:
                return _x('kyn_risk', 'Medium');

            case 4:
                return _x('kyn_risk', 'High');
            
            case 5:
                return _x('kyn_risk', 'Very High');
    ...
    // Kyndryl Customization End - dropdownRisk Choices

    // Kyndryl Customization Start - dropdownCloseCode Choices
    public static function dropdownCloseCode(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;
        ...
        $values[0] = static::getCloseCode(0);
        $values[1] = static::getCloseCode(1);
        $values[2] = static::getCloseCode(2);
        $values[3] = static::getCloseCode(3);
        $values[4] = static::getCloseCode(4);
        $values[5] = static::getCloseCode(5);
        $values[6] = static::getCloseCode(6);
        $values[7] = static::getCloseCode(7);
        $values[8] = static::getCloseCode(8);
        $values[9] = static::getCloseCode(9);
        $values[10] = static::getCloseCode(10);


        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getCloseCode($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_close_code', '');

            case 1:
                return _x('kyn_close_code', 'Successful');

            case 2:
                return _x('kyn_close_code', 'Successful with issues');
            
            case 3:
                return _x('kyn_close_code', 'Unsuccessful');

            case 4:
                return _x('kyn_close_code', 'Backout with issues');

            case 5:
                return _x('kyn_close_code', 'Backout successful');

            case 6:
                return _x('kyn_close_code', 'Rescheduled');

            case 7:
                return _x('kyn_close_code', 'Over ran');

            case 8:
                return _x('kyn_close_code', 'Cancelled');

            case 9:
                return _x('kyn_close_code', 'Unauthorized');

            case 10:
                return _x('kyn_close_code', 'Customer caused failure');
    ...
    // Kyndryl Customization End - dropdownCloseCode Choices

    // Kyndryl Customization Start - dropdownFailureProbability Choices
    public static function dropdownFailureProbability(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_failure_probability',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getFailureProbability(0);
        $values[2] = static::getFailureProbability(2);
        $values[3] = static::getFailureProbability(3);
        $values[4] = static::getFailureProbability(4);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getFailureProbability($value)
    {

        switch ($value) {
            case 0:
                return _x('kyn_failure_probability', '');

            case 4:
                return _x('kyn_failure_probability', 'High');

            case 2:
                return _x('kyn_failure_probability', 'Low');
            
            case 3:
                return _x('kyn_failure_probability', 'Medium');
    ...
    // Kyndryl Customization End - dropdownFailureProbability Choices

    // Kyndryl Customization Start - dropdownChangeType Choices
    public static function dropdownChangeType(array $options = [])
    {
        /** @var array $CFG_GLPI */
        global $CFG_GLPI;

        $p = [
            'name'     => 'kyn_change_type',
            'value'    => 0,
            'showtype' => 'normal',
            'display'  => true,
        ];
        ...
        $values[0] = static::getChangeType(0);
        $values[1] = static::getChangeType(1);
        $values[2] = static::getChangeType(2);
        $values[3] = static::getChangeType(3);

        return Dropdown::showFromArray($p['name'], $values, $p);
    }    

    public static function getChangeType($value)
    {

        // Kyndryl Customization -  #ADO 2073394 - Modification in existing Change Approval and Flow - Start
        switch ($value) {
            case 0:
                return _x('kyn_change_type', '');

            case Change::TYPE_NORMAL:
                return _x('kyn_change_type', 'Normal');

            case Change::TYPE_EMERGENCY:
                return _x('kyn_change_type', 'Emergency');
            
            case Change::TYPE_STANDARD:
                return _x('kyn_change_type', 'Standard');
    ...
    // Kyndryl Customization End - dropdownChangeType Choices

    // Kyndryl Customization Start - Revoke Validation
    public function manageValidationDelete($input)
    {
        $validation = $this->getValidationClassInstance();
        ...
        //Action for send_validation rule
        if (isset($input["_revoke_validation"])) {
            if (isset($input['entities_id'])) {
                $entid = $input['entities_id'];
            } else if (isset($this->fields['entities_id'])) {
                $entid = $this->fields['entities_id'];
            ...
            $validations_to_send = [];
            if (!is_array($input["_revoke_validation"])) {
                $input["_revoke_validation"] = [$input["_revoke_validation"]];
            }

            foreach ($input["_revoke_validation"] as $key => $value) {
                switch ($value) {
                    case 'revoke_all_validation':
                        if (
                            isset($input['id'])
                            && $input['id']
                        ) {         
                            $itemtype; // Kyndryl - captured rejection comment
							if (is_a($validation, ChangeValidation::class)) {
								$validation_list = $validation->find(['changes_id' => $input['id']]);
                                $itemtype = 'Change'; // Kyndryl - captured rejection comment
							} else {
								$validation_list = $validation->find(['tickets_id' => $input['id']]);
                                $itemtype = 'Ticket'; // Kyndryl - captured rejection comment
							}							
							foreach ($validation_list as $single_validation) {
                                // Kyndryl Customization Start - captured rejection comment
                                if( $single_validation['status'] == CommonITILValidation::ACCEPTED ){
                                    $content = $content . "<b>" . $single_validation['comment_submission'] . "</b> -> Approved | " . getUserName($single_validation['users_id_validate']) . " : " . strip_tags(Sanitizer::decodeHtmlSpecialChars($single_validation['comment_validation'])) . "<br/><br/>"; 
                                }
                                if( $single_validation['status'] == CommonITILValidation::REFUSED ){
                                    $content = $content . "<b>" . $single_validation['comment_submission'] . "</b> -> Rejected | " . getUserName($single_validation['users_id_validate']) . " : " . strip_tags(Sanitizer::decodeHtmlSpecialChars($single_validation['comment_validation'])) . "<br/></br>"; 
                                }
                                // Kyndryl Customization End - captured rejection comment
								$validation->check($single_validation['id'], PURGE);
								$validation->delete($single_validation, 1);									
							}
                            // Kyndryl Customization Start - captured rejection comment
                            $fup  = new ITILFollowup();	
                            $toadd = [
                            ...
                            //Kyndryl Customization Start - For Reset Button
                            if ($content !== null) {
                                $fup->add($toadd);
                            }
                            //Kyndryl Customization Start - For Reset Button
    ...
    // Kyndryl Customization End - Revoke Validation
    // Kyndryl Customization Start - Manage Assessment Delete
    public function manageAssessmsntDelete($input)
    {
        // Action for to delete change assessment 
        if (isset($input["_revoke_assessment"])) {
            if (isset($input['entities_id'])) {
                $entid = $input['entities_id'];
            } else if (isset($this->fields['entities_id'])) {
                $entid = $this->fields['entities_id'];
            ...
            $validations_to_send = [];
            if (!is_array($input["_revoke_assessment"])) {
                $input["_revoke_assessment"] = [$input["_revoke_assessment"]];
            }

            foreach ($input["_revoke_assessment"] as $key => $value) {
                switch ($value) {
                    case 'revoke_all_assessment':
                        if (
                            isset($input['id'])
                            && $input['id']
                        ) {         
							$changeassessmentobj = new ChangeAssessment();
                            $changeassessmentdatalist = $changeassessmentobj->find(['changes_id' => $input['id']]);
                            foreach($changeassessmentdatalist as $changeassessmentdata){
                                //Kyndryl Customization Start - For Reset Button
                                $changeassessmentobj->update([
                            ...
                            $this->update([
                                'id'    => $input['id'],
                                'assessment_completion_percentage' => '0',
                                'kyn_risk'  => '0',
                                'kyn_planned_start_date' => NULL,
                                'kyn_planned_end_date' => NULL,
                                'kyn_failure_probability' => '0',
                            ]);
                            //Kyndryl Customization Start - For Reset Button
                        }
                        break;
    ...
    public function cancelPendingTasks() 
    {
        ...
        $item_task_name   = $itemtype . 'Task';
        $item_task = new $item_task_name();

        $tasks = $item_task->find([
            $this->getForeignKeyField() => $this->fields['id'],
            'state' => [
                Planning::INFO,
                Planning::TODO,
        ...
        foreach($tasks as $task) {
            ...
            if($update) {
                Session::addMessageAfterRedirect(sprintf(__('%s (%s) task is cancelled'), Html::resume_text(RichText::getTextFromHtml($task['content'], false, true, true), 50), $task['id']));
            }
        }
    }
    // Kyndryl Customization -  #ADO 2073394 - Modification in existing Change Approval and Flow - End
    // Kyndryl Customization End - Manage Assessment Delete
    // Kyndryl Customization Start -  create problem ticket #2022419
    public function createProblemTicket($input)
    {
        // Action for to create problem ticket 
        if (isset($input["_add_problem"])) {
            if (isset($input['entities_id'])) {
                $entid = $input['entities_id'];
            } else if (isset($this->fields['entities_id'])) {
                $entid = $this->fields['entities_id'];
            ...
            $validations_to_send = [];
            if (!is_array($input["_add_problem"])) {
                $input["_add_problem"] = [$input["_add_problem"]];
            }
            foreach ($input["_add_problem"] as $key => $value) {
                switch ($value) {
                    case 'add_problem_ticket':
                        if (
                            isset($input['id'])
                            && $input['id']
                        ) {         
                            $problem_ticket_obj = new Problem_Ticket();
                            $is_problems = $problem_ticket_obj->find([
                                'tickets_id'    =>  $input['id']
                            ]);
                            if(empty($is_problems)){
                                $problem_obj = new Problem();
                                $problem_id = $problem_obj->add([
                                    'itilcategories_id' =>  $this->fields['itilcategories_id'],  // Kyndryl Customization Start - 2022419 - create problem ticket
                                    'name'              =>  $this->fields['name'],
                                    'content'           =>  strip_tags(Sanitizer::decodeHtmlSpecialChars($this->fields['name'])),
                                    'entities_id'       =>  $this->fields['entities_id']                                
                                ]);
                                // Kyndryl Customization Start - 2022419 - create problem ticket
                                if($this->grouplinkclass){
                                    $ticketgroupobj = new $this->grouplinkclass();
                                    $ticket_group = $ticketgroupobj->getActors($this->fields['id']);
                                    $ticket_group[2][0]['groups_id'];

                                    $problemgroupobj = new Group_Problem();
                                    $problemgroupobj->add([
                                        'problems_id' => $problem_id,
                                        'groups_id' => $ticket_group[2][0]['groups_id'], 
                                        'type'  => 2
                                    ]);
                                }  
                                // Kyndryl Customization End - 2022419 - create problem ticket
                                $problem_ticket_obj->add([
                                    'tickets_id'    =>  $input['id'],
                                    'problems_id'   =>  $problem_id
                                ]);                                
                            }                            
    ...
    // Kyndryl Customization End -  create problem ticket #2022419

    // Kyndryl Customization Start - 2109507 - GLPI ticket identifier required
    public function addTicketIdentifier($input)
    {
        if($this->getType() == 'Ticket') {
            $identifier;
            if(
                (isset($input['type']) && $input['type'] == Ticket::INCIDENT_TYPE)
                || (isset($this->fields['type']) && $this->fields['type'] == Ticket::INCIDENT_TYPE)
            ) {
                $identifier = 'IN-'.$this->fields['id'];
            }

            if(
                (isset($input['type']) && $input['type'] == Ticket::DEMAND_TYPE)
                || (isset($this->fields['type']) && $this->fields['type'] == Ticket::DEMAND_TYPE)
            ) {
                $identifier = 'SR-'.$this->fields['id'];
            }
            $this->update([
                'id'                    =>  $this->fields['id'],
                'kyn_ticket_identifier' =>  $identifier,
                '_disablenotif'         =>  false
            ]);    
        }
        ...
        if($this->getType() == 'Change') {            
            $this->update([
                'id'                    =>  $this->fields['id'],
                'kyn_change_identifier' =>  'CH-'.$this->fields['id'],
                '_disablenotif'         =>  false
            ]);    
        }
        ...
        if($this->getType() == 'Problem') {            
            $this->update([
                'id'                    =>  $this->fields['id'],
                'kyn_problem_identifier' =>  'PR-'.$this->fields['id'],
                '_disablenotif'         =>  false
            ]);    
        }
    ...
    // Kyndryl Customization End - 2109507 - GLPI ticket identifier required
}
